...
// @Injectable()
// export class BigStoreClientService {
...
export class Client {
    ...
    getProducts(): Promise<Product[]> {
        let url_ = this.baseUrl + "/BigStoreCustomerExtended/GetProducts";
        url_ = url_.replace(/[?&]$/, "");
    ...
    getCategories(): Promise<Category[]> {
        let url_ = this.baseUrl + "/BigStoreCustomerExtended/GetCategories";
        url_ = url_.replace(/[?&]$/, "");
     ...
     * @return Success
     */
    getWithSubCategories(categoryId: number | undefined): Promise<Category[]> {
        let url_ = this.baseUrl + "/BigStoreCustomerExtended/GetWithSubCategories?";
        if (categoryId === null)
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetWithSubCategories(_response);
        });
    }

    protected processGetWithSubCategories(response: Response): Promise<Category[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
